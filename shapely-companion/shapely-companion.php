...
define( 'SHAPELY_COMPANION', '1.2.9' );

/**
 * Initialize widgets and theme compatibility check.
 */
function shapely_companion_setup() {
...
add_action( 'after_setup_theme', 'shapely_companion_setup' );

/**
 * Initialize the plugin logic.
 */
function shapely_companion_init() {
	/**
	 ...
	 */
	function shapely_companion_setup_dashboard_widget() {
		shapely_companion_dashboard_widget();
	}
	add_action( 'wp_dashboard_setup', 'shapely_companion_setup_dashboard_widget' );
...
add_action( 'init', 'shapely_companion_init' );
